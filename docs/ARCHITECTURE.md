...
rejected, chosen = scored[0][1], scored[-1][1]   # keep only if chosen beats rejected
```
Sampling K *identical* prompts as one batch keeps RoPE positions aligned (no
padding), and the ~94% keep rate says the SFT model's outputs vary enough for
the judge to find a real preference most of the time.

### 2. The DPO objective (`dpo.py`)
...
masking idea as SFT), and the loss rewards preferences the reference *didn't
already* have:
```python
pol = logp(policy, chosen) - logp(policy, rejected)
ref = logp(ref,    chosen) - logp(ref,    rejected)   # no grad
loss = -logsigmoid(beta * (pol - ref)).mean()
```
`beta` (0.1) controls how far the policy may drift from the reference. One
line, no reward model, no RL loop.
...
between 💬 Instruct (SFT) and 🎯 Aligned (DPO) on the same three words.
